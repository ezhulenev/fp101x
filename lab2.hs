...
toDigits n = map parseInt $ show n
...
doubleSecond [] = []
doubleSecond xs = map f idx
  where idx = zip xs [1..]
        f (x, i) = if (even i) then x * 2 else x
...
isValid i = doubledSum `mod` 10 == 0
  where doubled = doubleSecond (toDigitsRev i)
        doubledSum = sumDigits doubled
